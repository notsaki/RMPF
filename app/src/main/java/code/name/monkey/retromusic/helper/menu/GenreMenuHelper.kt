...
object GenreMenuHelper : KoinComponent {
    ...
    fun handleMenuClick(activity: FragmentActivity, genre: GenreInfo, item: MenuItem): Boolean {
        when (item.itemId) {
            R.id.action_play -> {
    ...
    private fun getGenreSongs(genre: GenreInfo): List<Song> {
        return genreRepository.songs(genre.ids)
    }
}
